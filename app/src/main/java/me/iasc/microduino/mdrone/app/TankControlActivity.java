 ...
 * Bluetooth LE API.
 */
public class TankControlActivity extends AbstractBleControlActivity
        implements SensorEventListener, TankSettingFragment.OnFragmentInteractionListener {

    private final static String TAG = TankControlActivity.class.getSimpleName();
    ...
    private void readSetting() {
        rightHand = settings.getBoolean(TankSettingFragment.SET_RIGHT_HAND, true);

//        blockSendInterval = settings.getInt(TankSettingFragment.SET_BLOCK_SEND_INTERVAL, BLE_BLOCK_SEND_INTERVAL);
//        msgSendInterval = settings.getInt(TankSettingFragment.SET_BLE_SEND_INTERVAL, BLE_MSG_SEND_INTERVAL);
        bleTimerInterval = blockSendInterval + msgSendInterval + PROCESSING_TIME;

        currDeviceAddress = settings.getString(TankSettingFragment.SET_BLE_ADDRESS, null);
        String webCam = settings.getString(TankSettingFragment.SET_WEB_CAM_ADDRESS, "192.168.1.1:8080");
    ...
    private void showSettingFragment() {
        ...
        TankSettingFragment frag = TankSettingFragment.newInstance();
        frag.show(fm, "Tank Setting");
    }
        ...
        mTimer.schedule(new TimerTask() {
            ...
            public void run() {
                ...
                if (!sendingLock) {
                    ...
                    byte[] cmd = JoypadTankCommand.compose();
